...
<?php
    ...
    $datalist = array();
    print_r($resultQuery);
    if(trim($resultQuery['email']) !="" && password_verify($inputPassword, trim($resultQuery['password']))){
        print_r("YES");
        $result=1;
        $message = "เข้าสู่ระบบสำเร็จ";
        $datalist = array("id"=>$resultQuery['cus_id'],"fname"=>$resultQuery['fname'],"lname"=>$resultQuery['lname'],"email"=>$resultQuery['email'],"tel"=>$resultQuery['telephone'],"address"=>$resultQuery['address']);
