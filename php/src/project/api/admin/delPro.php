...
foreach ($referencingTables as $referencingTable) {
    ...
    #process
    $strSQL = "SELECT img FROM product WHERE pro_id = '" . mysqli_real_escape_string($conn, $inputId) . "'";
    $result = mysqli_query($conn, $strSQL);
    
    if ($row = mysqli_fetch_assoc($result)) {
        $imgFileName = $row['img'];

        $strSQL = "DELETE FROM product WHERE pro_id = '" . mysqli_real_escape_string($conn, $inputId) . "'";
        $query = mysqli_query($conn, $strSQL);

        // Additional code to delete the associated image file
        if ($query) {
            $imageFilePath = "../../assets/images/product/". $imgFileName; // Adjust the path accordingly
